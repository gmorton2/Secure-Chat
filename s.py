...
def auth(clienComSock):
	...
	while not flag:
		...
		if not userName in usernamePassword:
			...
			sigType=recvMsg(clienComSock)
			sigType=sigType.decode()
			print(userName, " has chosen the", sigType," scheme.")
...
def loadPublicKey(userName):
	if(usernameToSigType[userName]=="RSA"):
		...
		with open (userName+"dsa_public.pem", "rb") as pub_file:
			contents = pub_file.read()
			DSApubKey = DSA.importKey(contents)
			userToDSAPubKey[userName] = DSApubKey
...
def extractMsgAndVerifySig(msg, pubKey):
	...
	return retVal
#==============================================================================DSA verify	
def extractMsgAndVerifySigDSA(msg, pubKey):
	...
	try:
		...
		print("The DSA signature is valid!");
	# The verification failed
	except ValueError:
		print("Verification failed")
				...
				for sock in groupSocks:
					...
					if cliSock != sock:
						if(usernameToSigType[userName] == "RSA"):
							...
							#get the public key from the username
							DSApuKey = userToDSAPubKey[userName]
							...
							#verifies the signature and ectracts the user's sent message
							decMsg = extractMsgAndVerifySigDSA(decMsg, DSApuKey)
